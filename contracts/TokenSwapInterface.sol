...
/// Users can supply liquidity to earn transaction fees.  The tokens are of equal value.
/// The transaction fee should be set when the swap is created.
interface TokenSwapInterface {
  ...
  /// @param have The token the user is supplying
  /// @param want The token the user wants to receive
  /// @param amount The amount of token the user wants to swap
  /// @return The amount swapped, less the fee
  function swap(address have, address want, uint256 amount) external returns (uint256);
